...
class UnusedCodeFinder {
    ...
    extractNonExportedDeclarations(filePath) {
        ...
        const patterns = [
            ...
            { regex: /^(?!.*export)\s*class\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g, type: 'class' },
            {
                regex: /^(?!.*export)\s*(?:const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*(?:async\s+)?(?:function|\([^)]*\)\s*=>)/g,
                type: 'function',
            },
        ];
    ...
    shouldSkipName(name) {
        const skipPatterns = [
            /^_/, /^use[A-Z]/, /^handle[A-Z]/, /^on[A-Z]/, /^render[A-Z]/,
            /^get[A-Z]/, /^set[A-Z]/, /^is[A-Z]/, /^has[A-Z]/, /Config$/, /Options$/,
        ];
    ...
    extractImports(filePath) {
        ...
        for (const line of lines) {
            ...
            const namespaceImportMatch = line.match(/import\s+\*\s+as\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s+from/);
    ...
    addExport(name, type, file, line, filePath) {
        ...
        this.exports.get(name).push({
            name, type, file, line, filePath, isUsed: false, category: 'exported'
        });
    }
    ...
    addNonExportedDeclaration(name, type, file, line, filePath) {
        ...
        this.nonExportedDeclarations.set(key, {
            name, type, file, line, filePath, isUsed: false, category: 'non-exported'
        });
    }
